...
void renderFood(SDL_Renderer* renderer, int foodX, int foodY) {
    SDL_Rect destRect = { foodX, foodY, SNAKE_SIZE, SNAKE_SIZE };//rendering food at x and y cordinate and height and width
    SDL_RenderCopy(renderer, appleTexture, nullptr, &destRect); //rendering appletexture at the postion of destrect
}
...
void renderSTONE(SDL_Renderer* renderer, int stoneX, int stoneY) {
    SDL_Rect destRect = { stoneX, stoneY, SNAKE_SIZE, SNAKE_SIZE };
    SDL_RenderCopy(renderer, stoneTexture, nullptr, &destRect);
}
...
void renderLevelUp(SDL_Renderer* renderer, TTF_Font* font, const string& message) {
    ...
    string levelUpText1 = "Congo!! " + message;
    string levelUpText2 = message == "level 2" ? "Be aware of the RUSSELL's VIPER ." : "Be aware of the stone.";
...
void renderSTONETimer(SDL_Renderer* renderer, TTF_Font* font, Uint32 STONESpawnTime, Uint32 STONELifeTime) {
    Uint32 currentTime = SDL_GetTicks();
    Uint32 elapsedTime = currentTime - STONESpawnTime;
    Uint32 remainingTime = STONELifeTime - elapsedTime;

    if (remainingTime > 0) {
        SDL_Color textColor = { 0, 0, 0, 255 };
        string timerText = "STONE disappears in: " + to_string(remainingTime / 1000) + "s";
        SDL_Surface* textSurface = TTF_RenderText_Solid(font, timerText.c_str(), textColor);
        SDL_Texture* textTexture = SDL_CreateTextureFromSurface(renderer, textSurface);
        int textWidth = textSurface->w;
        int textHeight = textSurface->h;
...
bool checkSTONECollision(int stoneX, int stoneY, const SnakeSegment& head) {
    return head.x == stoneX && head.y == stoneY;
}
...
void generateSTONE(int& stoneX, int& stoneY, const vector<SnakeSegment>& snake, const vector<SDL_Rect>& obstacles, const RandomSnake& randomSnake) {
    bool validPosition = false;
    while (!validPosition) {
        validPosition = true;
        stoneX = (rand() % (SCREEN_WIDTH / SNAKE_SIZE)) * SNAKE_SIZE;
        stoneY = (rand() % (SCREEN_HEIGHT / SNAKE_SIZE)) * SNAKE_SIZE;
        for (const auto& segment : snake) {
            if (segment.x == stoneX && segment.y == stoneY) {
                validPosition = false;
                break;
            }
        }
        for (const auto& obstacle : obstacles) {
            if (obstacle.x == stoneX && obstacle.y == stoneY) {
                validPosition = false;
                break;
            }
        }
...
//level up =677 ,banana span after 711
int main(int argc, char* args[]) {
    ...
    Uint32 STONESpawnTime=0;
    bool bananaActive = false;
    bool stoneactive=false;
    const Uint32 bananaLifetime = 4000;
    const Uint32 STONELifeTime=4000;
    ...
    while (!quit) {
        ...
        if (state == PLAYING) {
            ...
            if (checkFoodCollision(foodX, foodY, snake.front())) {
                ...
                if (!levelUpTriggered && score >= 100 && currentLevel == "level 1") {
                    state = LEVEL_UP;
                    levelUpStartTime = SDL_GetTicks();
                    levelUpTriggered = true;
                    currentLevel = "level 2";
                } else if (!levelUpTriggered && score >= 150 && currentLevel == "level 2") {
                    state = LEVEL_UP;
                    levelUpStartTime = SDL_GetTicks();
            ...
            if (stoneactive && checkSTONECollision(stoneX, stoneY, snake.front())) {
                grow = true;
                score -= 10;
                stoneactive = false;
                pointsincelaststone = 0;
            }
            ...
            if (score >= 4 && pointsSinceLastBanana >= 3 && !bananaActive) {
                generateBanana(bananaX, bananaY, snake, obstacles, randomSnake);
                bananaSpawnTime = SDL_GetTicks();
                bananaActive = true;
            }
            if (score >= 4 && pointsincelaststone >= 3 && !stoneactive) {
                generateSTONE(stoneX, stoneY, snake, obstacles, randomSnake);
                STONESpawnTime = SDL_GetTicks();
                stoneactive = true;
            }
            ...
            if (stoneactive && SDL_GetTicks() - STONESpawnTime >= STONELifeTime) {
                bananaActive = false;
            }
            ...
            if (stoneactive) {
                renderSTONE(renderer, stoneX, stoneY);
                renderSTONETimer(renderer, font, STONESpawnTime, STONELifeTime);
            }
